...
def pipeline_llm(
    ...
    module_name_per_stage = job_config.parallelism.module_fqns_per_model_part

    if module_name_per_stage is None:

        # get list[list[str]] for module names in each stage
        module_name_per_stage = generate_llm_fqn_per_model_part(
            num_virtual_stages, num_layers, input_weight, output_weight
        )

    for i, stage_ms in enumerate(module_name_per_stage):
...
def build_pipeline_schedule(
    ...
    if n_microbatches < num_total_stages:
        logger.warning(
            f"Number of microbatches ({n_microbatches}) is less than total number "
...
def pipeline_module_split(
    ...
    if style == "v":
        ...
        raise ValueError(f"Unkown schedule style {style}")
